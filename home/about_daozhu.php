	...
	<body>
		<div class="header" id="home">
			...
			<div class="header-banner">
					<!-- Top Navigation -->
					<section class="bgi banner5"><h2>导助名单</h2> </section>
				
				<!--detail -->

				<div class="container">					
					<div class="panel panel-primary">
						<div class="panel-heading">2015级指导员助理汇总表_公示</div>
						<div class="panel-body">
							<table class="table table-bordered table-striped">
								<tr>
								 ...
								 <?php
					  		      $sql = "select * from tb_daozhu";
					  		      foreach ($pdo->query($sql) as $row) {					  		      	
				  		      	    echo "<tr><td>{$row['A']}</td><td>{$row['B']}</td><td>{$row['C']}</td><td>{$row['D']}</td><td>{$row['E']}</td><td>{$row['F']}</td><td>{$row['G']}</td><td>{$row['H']}</td><td>{$row['所属中队名称']}</td><td>{$row['专业班级名称']}</td></tr>";
